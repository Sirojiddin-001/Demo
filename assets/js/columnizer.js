...
Element.prototype.dbabych_columnize = function (options) {
  this.defaults = {
    // Ширина колонки в пкс
    width: 360,
    // Высота колонки в пкс
    height: 700,
    // Класс стиля, который прописывается для колонок
    col_class: getCookie("page") || document.getElementById("page-select").value,
    // Объект, содержащий поток колонок, для управления в функции done_func
    cols_block_obj: false,
    // Функция, которая будет запущена после разбиения на колонки
    done_func: function () {},
    // Блок, куда записываются колонки в результате разбивки
  ...
  Element.prototype.columnize_init = function () {
    ...
    this.debug.style.fontSize =
      getCookie("font_size") ||
      document.querySelector("#font-size-select").value;
    this.debug.style.fontFamily =
      getCookie("font_family") || document.querySelector("#font-select").value;
  ...
  Element.prototype.columnize_node = function (from, to, debug) {
    ...
    while (from.childNodes.length > 0) {
      ...
      if (
        from.firstChild.nodeType == document.ELEMENT_NODE ||
        from.firstChild.nodeType == document.TEXT_NODE
      ) {
        ...
        if (debug.get_offsetHeight() > this.params.height) {
          ...
          if (from.firstChild.nodeType == document.ELEMENT_NODE) {
            ...
            if (
              ...
              result_inline = this.columnize_node(
                from.firstChild,
                to.lastChild,
                debug
              );
            ...
            for (i = text_array.length - 1; i > 0; i--) {
              ...
              if (debug.get_offsetHeight() > this.params.height) {
                ...
                from.firstChild.nodeValue = text_array
                  .slice(i, text_array.length)
                  .join(" ");
